...
Every pack also includes a compact **Context Manifest**. In JSON output this is
...
- MCP `cargo-context://manifest` exposes scrubbed provenance metadata without raw secret values.
...
| Resource | Returns |
| :--- | :--- |
| `cargo-context://diff` | Structured `Diff` JSON |
| `cargo-context://errors` | Structured `Diagnostics` JSON |
| `cargo-context://map` | Structured `WorkspaceMap` JSON |
| `cargo-context://manifest` | Structured pack provenance JSON |
...
Scrubbing, token budgeting, macro expansion, pack provenance, and `.cargo-context/scrub.yaml` auto-discovery all apply identically in MCP mode — the protocol boundary does not relax the guarantees. Diagnostics go to stderr via `tracing`; stdout stays a clean JSON-RPC channel.
