...
fn apply_priority(candidates: Vec<(Priority, Section)>, limit: usize) -> Allocation {
    ...
    let mut decisions: Vec<BudgetDecision> = kept
        .iter()
        .map(|s| BudgetDecision {
            section: s.name.clone(),
            priority: P_EXEMPT,
            original_tokens: s.token_estimate,
            final_tokens: s.token_estimate,
            outcome: BudgetOutcome::Kept,
            reason: Some("exempt".into()),
        })
        .collect();
    let mut running: usize = 0;

    for (p, s) in competing {
        let original_tokens = s.token_estimate;
        if running + s.token_estimate <= remaining {
            ...
            decisions.push(BudgetDecision {
                ...
                original_tokens,
                final_tokens: 0,
                outcome: BudgetOutcome::Dropped,
                reason: Some("priority budget exhausted".into()),
            });
            dropped.push(s.name);
        }
    }
...
fn apply_truncate(
    ...
    let mut iter = competing.into_iter();
    while let Some((p, mut s)) = iter.next() {
        let original_tokens = s.token_estimate;
        ...
        if budget_left == 0 {
            decisions.push(BudgetDecision {
                ...
                final_tokens: 0,
                outcome: BudgetOutcome::Dropped,
                reason: Some("truncate budget exhausted".into()),
            });
            dropped.push(s.name);
            continue;
        }
        ...
        decisions.push(BudgetDecision {
            ...
            original_tokens,
            final_tokens: s.token_estimate,
            outcome: BudgetOutcome::Truncated,
            reason: Some("hard truncated to remaining budget".into()),
        });
        kept.push(s);
        ...
        for (p, s) in iter {
            decisions.push(BudgetDecision {
                ...
                final_tokens: 0,
                outcome: BudgetOutcome::Dropped,
                reason: Some("truncate stops after first overflow".into()),
            });
            dropped.push(s.name);
        }
        break;
    ...
    for (p, mut s) in competing {
        ...
        if target_tokens == 0 {
            ...
            decisions.push(BudgetDecision {
                ...
                final_tokens: 0,
                outcome: BudgetOutcome::Dropped,
                reason: Some("proportional share rounded to zero".into()),
            });
            dropped.push(s.name);
            continue;
        }
        ...
        if body_tokens == 0 {
            decisions.push(BudgetDecision {
                ...
                final_tokens: 0,
                outcome: BudgetOutcome::Dropped,
                reason: Some("proportional share cannot fit truncation marker".into()),
            });
            dropped.push(s.name);
            continue;
        }
        ...
        decisions.push(BudgetDecision {
            ...
            final_tokens: s.token_estimate,
            outcome: BudgetOutcome::Truncated,
            reason: Some("proportionally truncated".into()),
        });
        kept.push(s);
    }
...
mod tests {
    ...
    fn priority_drops_lowest_priority_when_over_budget() {
        ...
        assert_eq!(a.decisions[2].outcome, BudgetOutcome::Dropped);
    }
    ...
    fn truncate_hard_cuts_overflowing_section() {
        ...
        assert_eq!(a.decisions[1].outcome, BudgetOutcome::Truncated);
    }
    ...
    fn proportional_scales_all_sections_when_over_budget() {
        ...
        assert!(
            a.decisions
                .iter()
                .all(|d| d.outcome == BudgetOutcome::Truncated),
            "all decisions should report truncation"
        );
        // Budget is honored.
        assert!(
