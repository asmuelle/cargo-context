...
use crate::budget::{
    self, Budget, BudgetDecision, BudgetStrategy, P_DIFF, P_ENTRY, P_ERROR, P_EXEMPT, P_MAP,
    P_TESTS, Priority,
};
...
impl Pack {
    ...
    pub fn render_markdown(&self) -> String {
        ...
        out.push_str("## 🧾 Context Manifest\n\n");
        out.push_str(&self.manifest.render_markdown());
        out.push_str("\n\n");
    ...
    pub fn render_xml(&self) -> String {
        ...
        out.push_str(&self.manifest.render_plain_indented("    "));
        out.push_str("  </manifest>\n");
        for s in &self.sections {
            out.push_str(&format!(
                "  <section name=\"{}\" tokens=\"{}\">\n{}\n  </section>\n",
...
pub struct PackManifest {
    ...
    pub path_filters: PathFilterManifest,
    pub files: Vec<FileProvenance>,
    pub budget: BudgetManifest,
    pub scrub: ScrubManifest,
}
...
impl PackManifest {
    fn render_markdown(&self) -> String {
        ...
        out.push_str(&format!(
            "- Budget: `{}` strategy, {}/{} tokens",
            self.budget.strategy.label(),
        ...
        let changed = self
            ...
            .decisions
            .iter()
            .filter(|d| !matches!(d.outcome, budget::BudgetOutcome::Kept))
            .count();
        if changed > 0 {
            out.push_str(&format!(", {changed} section decision(s)"));
        }
        out.push('\n');
        out.push_str(&format!(
            "- Scrub: {} redaction(s){}\n",
            self.scrub.redactions,
        ...
        if !self.path_filters.include.is_empty() {
            out.push_str(&format!(
                "- Include filters: `{}`\n",
                self.path_filters.include.join("`, `")
            ));
        }
        if !self.path_filters.exclude.is_empty() {
            out.push_str(&format!(
                "- Exclude filters: `{}`\n",
                self.path_filters.exclude.join("`, `")
            ));
        }
        let included = self
            .files
            .iter()
            .filter(|f| matches!(f.status, FileProvenanceStatus::Included))
            .count();
        let suppressed = self
            .files
            .iter()
            .filter(|f| matches!(f.status, FileProvenanceStatus::Suppressed))
            .count();
        if included > 0 || suppressed > 0 {
            out.push_str(&format!(
                "- Files: {included} included, {suppressed} suppressed\n"
            ));
        }
        out
    }

    fn render_plain_indented(&self, indent: &str) -> String {
...
pub struct FileProvenance {
    ...
    pub status: FileProvenanceStatus,
    pub reason: Option<String>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum FileProvenanceStatus {
    Included,
    Suppressed,
}
...
impl PackBuilder {
    ...
    pub fn build(self) -> Result<Pack> {
        ...
        let mut provenance = ProvenanceBuilder::new(&root);
        ...
        for path in &error_files {
            provenance.record(path, "compiler_error", &path_filters);
        }
        ...
        let diff = if wants.diff || wants.tests {
            collect::git_diff(&root, self.diff_range.as_deref())
                ...
                .map(|d| {
                    for f in &d.files {
                        provenance.record(&f.path, "diff", &path_filters);
                    }
        ...
        if wants.entry
            && let Some((content, paths)) = try_collect_entry(&root, &path_filters)
        {
            for path in paths {
                provenance.record_included(&path, "entry_point");
            }
            candidates.push((
                P_ENTRY,
        ...
        for path in &self.files_from {
            provenance.record(path, "files_from", &path_filters);
        }
        ...
        for finding in &self.impact_findings {
            let source = finding
                .id
                .as_ref()
                .map(|id| format!("impact_scope:{id}"))
                .unwrap_or_else(|| "impact_scope".to_string());
            provenance.record(&finding.primary_path, &source, &path_filters);
        }
        ...
        for path in &force_include_paths {
            provenance.record_included(path, "include_path");
        }
        ...
        if wants.tests {
            ...
            if !changed.is_empty()
                && let Some((content, paths)) = try_collect_tests(&root, &changed)
            {
                for path in paths {
                    provenance.record(&path, "related_test", &path_filters);
                }
                candidates.push((
                    P_TESTS,
        ...
        {
            for path in paths {
                provenance.record_included(&path, "macro_expansion");
            }
            candidates.push((
                P_ENTRY,
...
fn try_collect_tests(
    ...
    if rt.is_empty() {
        ...
        let paths = rt.files.iter().map(|f| f.path.clone()).collect();
        Some((render_tests(&rt), paths))
    }
}

fn try_collect_entry(
    root: &Path,
    path_filters: &UserPathFilters,
) -> Option<(String, Vec<PathBuf>)> {
    ...
    if ep.is_empty() {
        ...
        let paths = ep.files.iter().map(|f| f.path.clone()).collect();
        Some((render_entry(&ep), paths))
    }
}
...
struct ProvenanceBuilder {
    root: PathBuf,
    files: BTreeMap<String, FileTrace>,
}

#[derive(Debug, Default)]
struct FileTrace {
    sources: BTreeSet<String>,
    status: Option<FileProvenanceStatus>,
...
impl ProvenanceBuilder {
    ...
    fn record(&mut self, path: &Path, source: &str, filters: &UserPathFilters) {
        let path_key = self.path_key(path);
        let allowed = filters.allows(path);
        let trace = self.files.entry(path_key).or_default();
        trace.sources.insert(source.to_string());
        if allowed {
            trace.status = Some(FileProvenanceStatus::Included);
        } else {
            trace.status = Some(FileProvenanceStatus::Suppressed);
            trace.reason = Some("--exclude-path".into());
        }
    }

    fn record_included(&mut self, path: &Path, source: &str) {
        let path_key = self.path_key(path);
        let trace = self.files.entry(path_key).or_default();
        trace.sources.insert(source.to_string());
        trace.status = Some(FileProvenanceStatus::Included);
    ...
    fn finish(self) -> Vec<FileProvenance> {
        self.files
            .into_iter()
            .map(|(path, trace)| FileProvenance {
                path,
                sources: trace.sources.into_iter().collect(),
                status: trace.status.unwrap_or(FileProvenanceStatus::Included),
                reason: trace.reason,
            })
            .collect()
    }
...
mod tests {
    ...
    #[test]
    fn manifest_records_diff_range_filters_and_budget_decisions() {
        let tmp = tempfile::tempdir().unwrap();
        let pack = PackBuilder::new()
            .preset(Preset::Fix)
            .diff_range("HEAD~1..HEAD")
            .include_path("src/lib.rs")
            .exclude_path("src/secret.rs")
            .budget(Budget {
                max_tokens: 10,
                reserve_tokens: 0,
        ...
        assert_eq!(pack.manifest.preset, Preset::Fix);
        assert!(matches!(
            pack.manifest.diff_source,
            DiffSource::Range(ref range) if range == "HEAD~1..HEAD"
        ));
        assert_eq!(pack.manifest.path_filters.include, vec!["src/lib.rs"]);
        assert_eq!(pack.manifest.path_filters.exclude, vec!["src/secret.rs"]);
        assert_eq!(pack.manifest.budget.strategy, BudgetStrategy::Priority);
        assert!(
    ...
    #[test]
    fn provenance_records_suppressed_and_multi_source_files() {
        let tmp = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(tmp.path().join("src")).unwrap();
        std::fs::write(tmp.path().join("src/lib.rs"), "pub fn lib() {}\n").unwrap();
        std::fs::write(tmp.path().join("src/secret.rs"), "pub fn secret() {}\n").unwrap();

        let findings = vec![Finding {
            id: Some("f-lib".into()),
            primary_path: PathBuf::from("src/lib.rs"),
            kind: None,
            confidence: Some(0.9),
            severity: None,
            tier: None,
        ...
        let lib = pack
            ...
            .expect("lib provenance");
        assert_eq!(lib.status, FileProvenanceStatus::Included);
        assert!(lib.sources.contains(&"files_from".to_string()));
        assert!(lib.sources.contains(&"impact_scope:f-lib".to_string()));

        let secret = pack
            .manifest
            .files
            .iter()
            .find(|f| f.path == "src/secret.rs")
            .expect("secret provenance");
        assert_eq!(secret.status, FileProvenanceStatus::Suppressed);
        assert_eq!(secret.reason.as_deref(), Some("--exclude-path"));
    }
